...
var Catalog = {
	...
	init: function() {
		...
		$('#js-csearch').on('input', function() {
			//Store.set('catalog-search-query',this.value);
	...
	debounce  : function(delay, fn, args) {
		setTimeout(function () {Catalog.search(args)}, delay);
	},
	...
	getdata: function(callback,filter){
		...
		$.getJSON(
			...
			.then(function() {Catalog.onloadsearch();}); //callback() при первичной загрузке json выполняем попытку поиска, если в окне запроса что-то было введено. Если было пусто, то вызовется обычный display()
	},
	display: function(data){
		...
		$.each(data, function(index, thread) {
			...
			//htmlcode += '<div id="thread_' + thread.num + '" class="cthread">';
			...
			// if(thread.email) {
			// 	htmlcode += '<a href="' + thread.email + '">' + thread.name + '</a>';
			// } else {
			// 	htmlcode += thread.name + '&nbsp;';
			// }
			switch (thread.trip) {
				case '!!%adm%!!':        htmlcode += '<span class="adm">## Abu ##<\/span>'; break;
				case '!!%mod%!!':        htmlcode += '<span class="mod">## Mod ##<\/span>'; break;
				case '!!%Inquisitor%!!': htmlcode += '<span class="inquisitor">## Applejack ##<\/span>'; break;
				case '!!%coder%!!':      htmlcode += '<span class="mod">## Кодер ##<\/span>'; break;
				default:                 htmlcode += '<span class="postertrip">' + thread.trip + '<\/span>';
			}
			htmlcode += '<span>Постов: ' + thread.posts_count + ' / Файлов: ' + thread.files_count + '</span> ';
			//htmlcode += '<span style="border:none;">И: ' + thread.files_count + '</span>';
			if(thread.tags) {
				tag = ' /' + thread.tags + '/';
			}
			...
			//if(thread.banned == 1) htmlcode += '			<br/><span class="pomyanem">(Автор этого поста был забанен. Помянем.)</span>';
			//else if(thread.banned == 2) htmlcode += '	<br/><span class="pomyanem">(Автор этого поста был предупрежден.)</span>';
			htmlcode += '</div>';
			htmlcode += '</div>'
...
$('#js-threads').on('mouseenter', '.ctlg__thread', (e) => {
	...
	if(window.innerWidth > 640 && left - 80 < 0) newel.style.left = '80px'; //-80px margin
	if(window.innerWidth > 640 && left - 80 + 380 > window.innerWidth) { //380 box width
